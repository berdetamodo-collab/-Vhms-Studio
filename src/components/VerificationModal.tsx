    ...
    >
      ...
      >
        ...
        <div className="p-3 border-t border-slate-700 text-right bg-slate-900/50 rounded-b-lg">
          <button onClick={onClose} className="bg-amber-500 text-slate-900 font-semibold py-2 px-6 rounded-md hover:bg-amber-600 transition-colors">Tutup</button>
        </div>
      </div>
      <style>{`@keyframes fadeInScale { from { transform: scale(0.95); opacity: 0; } to { transform: scale(1); opacity: 1; } } .animate-fade-in-scale { animation: fadeInScale 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; }`}</style>
    </div>
  );
};